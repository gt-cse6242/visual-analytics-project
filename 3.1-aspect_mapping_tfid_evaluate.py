...
def process_row(row: Iterable[Row]) -> Iterable[Row]:
    ...
    d['aspect_baseline'] = route_aspect(row['aspect_seed'], MAPPINGS, ASPECTS)
    return Row(**d)
    return Row(**d)
...
print("\n========== Apply Mapping tf-idf and cosine similarity ======================================================")
...
# df_output_baseline_not_null = df_output.filter((F.col("aspect_baseline").isNotNull()) | (F.length(F.col("aspect_baseline")) != 0))
# print(f"Baseline: row with no aspect: {df_output.count() - df_output_baseline_not_null.count()} over total row count {df_output.count()}")
# df_output_tf_idf_not_null = df_output.filter((F.col("aspect").isNotNull()) | (F.length(F.col("aspect")) != 0))
# print(f"TF-IDF: Row with no aspect: {df_output.count() - df_output_tf_idf_not_null.count()} over total row count {df_output.count()}")
